...
{
    private static function utility(): MakeupUtility
    {
        return new MakeupUtility();
    }
    ...
    private static function ctx(array $over = []): MakeupContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new MakeupContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new MakeupOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MakeupError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MakeupOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MakeupMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MakeupOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MakeupMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MakeupOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MakeupMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MakeupSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MakeupMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new MakeupResponse([]),
            'result' => new MakeupResult([]),
        ]);
        [, $err] = MakeupMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new MakeupSpec(['step' => 's']),
            'response' => new MakeupResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MakeupResult([]),
        ]);
        [, $err] = MakeupMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MakeupSpec(['step' => 's']),
            'response' => new MakeupResponse(self::resp(200, ['v' => 1])),
            'result' => new MakeupResult([]),
        ]);
        [, $err] = MakeupMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MakeupSpec(['step' => 's']),
            'response' => new MakeupResponse(self::resp(200, ['v' => 2])),
            'result' => new MakeupResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MakeupMakeResponse::call($ctx);
    ...
    {
        $preset = new MakeupResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MakeupSpec([]),
            'response' => new MakeupResponse([]),
            'result' => new MakeupResult([]),
        ...
        [$got, $err] = MakeupMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MakeupResult([])]);
        [, $err] = MakeupMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MakeupSpec([]), 'result' => null]);
        [, $err] = MakeupMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MakeupSpec(['step' => 's']),
            'result' => new MakeupResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MakeupOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MakeupMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MakeupSpec(['step' => 's']),
            'result' => new MakeupResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MakeupOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MakeupMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MakeupResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MakeupSpec([]),
            'result' => new MakeupResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MakeupMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MakeupMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new MakeupError('boom', 'boom');
        $utility->fetcher = function (MakeupContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MakeupSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MakeupMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MakeupContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = MakeupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(MakeupResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MakeupContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MakeupSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MakeupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MakeupResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MakeupSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MakeupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = MakeupMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MakeupMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MakeupResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MakeupDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MakeupResult(['ok' => false])]);
        $this->expectException(MakeupError::class);
        MakeupDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new MakeupResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MakeupMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MakeupBaseFeature();
        $b = new MakeupBaseFeature();
        MakeupFeatureAdd::call($ctx, $a);
        MakeupFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MakeupBaseFeature
    {
        $f = new MakeupBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        MakeupFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): MakeupContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MakeupSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MakeupPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new MakeupResponse(['headers' => null]),
            'result' => new MakeupResult([]),
        ]);
        MakeupResultHeaders::call($ctx);
