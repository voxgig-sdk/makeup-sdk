...
boru-driven command-line client **and** interactive REPL for the Makeup
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/makeup-cli)
make build
...
MAKEUP_BASE=https://api.example.com ./makeup-cli list product
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/makeup-cli
   ```
...
Run with no arguments to open a REPL (prompt `makeup>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/makeup-cli`. |
...
(`./makeup-cli <expr>`) and interactively (the REPL), and expressions compose the
