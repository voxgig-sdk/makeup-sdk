...
class MakeupNetsimFeature extends MakeupBaseFeature
{
    ...
    private function _simulate(MakeupContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(MakeupContext $ctx, array $applied): void
    {
        $client = $this->client;
