...
class FtCtrl extends MakeupControl
{
...
class FtClient
{
    ...
    private MakeupUtility $utility;

    public function __construct(MakeupUtility $utility, array $options)
    {
    ...
    public function get_utility(): MakeupUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MakeupContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MakeupContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MakeupConfig::make_feature($name);
    ...
    private static function buildUrl(MakeupSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MakeupError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MakeupResponse($fetched);
        }
    ...
    private function fail(MakeupContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MakeupContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
