...
// Makeup SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MakeupOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MakeupOmni::structfix($now, Struct::undef()))
                        !== json_encode(MakeupOmni::structfix($val, Struct::undef()))) {
