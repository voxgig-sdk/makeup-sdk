...
//    only - a typed MakeupContext there would make every ctx
...
final class MakeupOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MakeupContext($clean, null);
    ...
    public static function err_from_map($m): ?MakeupError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MakeupError(is_string($code) ? $code : '', $msg);
    }
