...
class ProductEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = MakeupSDK::test($seed, null);
        $seen = iterator_to_array($base->Product(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = MakeupConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = MakeupSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAKEUP_TEST_PRODUCT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function product_basic_setup($extra)
{
    ...
    $client = MakeupSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("MAKEUP_TEST_PRODUCT_ENTID");
    ...
    if ($env["MAKEUP_TEST_LIVE"] === "TRUE") {
        ...
        $client = new MakeupSDK(Helpers::to_map($merged_opts) ?? []);
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["MAKEUP_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
