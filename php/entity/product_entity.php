...
class ProductEntity
{
    ...
    {
        if ($args) {
            $this->_data = MakeupHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = MakeupHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws MakeupError on failure (item-5 convention).
     */
