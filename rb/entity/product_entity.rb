...
class ProductEntity
  ...
  def data_set(args)
    if args
      @_data = MakeupHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MakeupHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Product>, Array] the matching Product items; raises MakeupError on failure
