...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = MakeupOmni.make_runner(TEST_JSON_FILE, MakeupSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : MakeupOmni.errify(err)
    end
