...
The Ruby SDK for the Makeup API — an entity-oriented client using idiomatic Ruby conventions.
...
begin
  ...
  products.each do |item|
    puts "#{item["id"]} #{item["api_featured_image"]}"
  end
rescue => err
...
raise a `MakeupError` (a `StandardError` subclass), so wrap
