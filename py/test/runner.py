# Makeup SDK test runner

from __future__ import annotations
import os
import json

from makeup_sdk.utility.voxgig_struct import voxgig_struct as vs


class MakeupTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MakeupTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MakeupTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MakeupTestRunner.getenv("MAKEUP_TEST_LIVE")
        override = MakeupTestRunner.getenv("MAKEUP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MakeupTestRunner.getenv(key)
        ...
        explain = MakeupTestRunner.getenv("MAKEUP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MAKEUP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MakeupTestRunner._test_control is not None:
            return MakeupTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MakeupTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MakeupTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MakeupTestRunner.load_test_control()
...
def load_env_local():
    MakeupTestRunner.load_env_local()


def env_override(m):
    return MakeupTestRunner.env_override(m)


def entity_data(v):
    return MakeupTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MakeupTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MakeupTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MakeupTestRunner.load_test_control()


def live_client_options():
    return MakeupTestRunner.live_client_options()


def live_delay_ms():
    return MakeupTestRunner.live_delay_ms()
