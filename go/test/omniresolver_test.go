...
import (
	...
	omni "github.com/voxgig-sdk/makeup-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MakeupError); is {
				out := map[string]any{
					"name":    "MakeupError",
...
func omniCtx(arg any, client *sdk.MakeupSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MakeupSDK); is {
			client = live
