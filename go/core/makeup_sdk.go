...
import (
	...
	vs "github.com/voxgig-sdk/makeup-sdk/go/utility/struct"
)

type MakeupSDK struct {
	Mode     string
...
func (sdk *MakeupSDK) OptionsMap() map[string]any {
...
func (sdk *MakeupSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *MakeupSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *MakeupSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MakeupSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *MakeupSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *MakeupSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("MakeupSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *MakeupSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MakeupSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("MakeupSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
