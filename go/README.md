...
The Golang SDK for the Makeup API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/makeup-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/makeup-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/makeup-sdk/go=../makeup-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *MakeupSDK
```
...
| `Product` | `(data map[string]any) MakeupEntity` | Create a Product entity instance. |

### Entity interface (MakeupEntity)

All entities implement the `MakeupEntity` interface.
...
| `product_colors` | `[]any` | Available color variants of the product |
...
| `rating` | `float64` | Average rating of the product (0-5) |
| `tag_list` | `[]any` | List of tags associated with the product (e.g., Vegan, Organic, Gluten Free) |
...
```
github.com/voxgig-sdk/makeup-sdk/go/
├── makeup.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/makeup-sdk/go`) re-exports everything needed
