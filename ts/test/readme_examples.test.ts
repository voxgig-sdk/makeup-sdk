...
//     (\`new MakeupSDK(...)\` / \`MakeupSDK.test(...)\` become
//     \`MakeupSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new MakeupSDK(...)\` and any
// \`MakeupSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MakeupSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MakeupSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
