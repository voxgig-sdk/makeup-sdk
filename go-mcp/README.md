...
[MCP](https://modelcontextprotocol.io) server exposing the Makeup SDK as
two agent tools — `makeup_list` and `makeup_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/makeup-mcp)
make build

# 2. Provide credentials via the environment
export MAKEUP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user makeup \
  -- /absolute/path/to/makeup-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./makeup-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// makeup_list: first page of records
{ "entity": "product" }
{ "entity": "product", "query": { } }

// makeup_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/makeup-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user makeup \
     -- "$PWD"/dist/*/makeup-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `makeup_list` and `makeup_load` tools now appear
   in new sessions. Ask the agent to *"list product using makeup"*
   and it calls `makeup_list` with `{"entity":"product"}`.
...
```sh
export MAKEUP_APIKEY=sk_live_xxx            # API key
export MAKEUP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `makeup_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `makeup_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MAKEUP_APIKEY` | API key sent with every request. |
| `MAKEUP_BASE` | Optional override of the API base URL. |
...
```sh
./makeup-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"makeup_load","arguments":{"entity":"product","query":{"id":1}}}}'
```
