...
MCP server exposing the Makeup SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `makeup_list` | `entity`, optional `query` map | First page of records as JSON |
| `makeup_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `makeup_list` and
`makeup_load` tools become available in new sessions.
